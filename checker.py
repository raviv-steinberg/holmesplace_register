...
def fetch_notifier_tuple(file: str):
    return_code, output = subprocess.getstatusoutput(
        cmd=f'python3.11 {NOTIFY_REGISTRATION_SCRIPT} -f {file} -t {args.threshold}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "This script notifies the user when a lesson registration time"
            " is nearing based on the specified threshold value.\n"
            "If the threshold is reached, it attempts to register the "
            "user for the lesson using provided user data.\n\n"
            "The script uses two sub-scripts for this process:\n"
            "   1. NOTIFY_REGISTRATION_SCRIPT:"
            " Checks if lesson registration is nearing based on threshold.\n"
            "   2. REGISTER_LESSON_SCRIPT:"
            " Registers the user to the lesson if the threshold is met.\n"
        ),
        epilog=(
            "Example:\n"
    ...
    lesson_id, minutes = fetch_notifier_tuple(file=user_data_service.filepath)
    ...
    if lesson_id and minutes:
        ...
        if not is_in_progress:
            ...
            #result = subprocess.Popen(['python3.11', REGISTER_LESSON_SCRIPT, '-l', lesson_id, '-u', user_data_service.name])
            # result=subprocess.getstatusoutput(cmd=f'python3.11 {REGISTER_LESSON_SCRIPT} -l {lesson_id} -u {user_data_service.name}')
            manager = LessonRegistrationManagerFactory(user_data_service=user_data_service, lesson_id=lesson_id).get()
            result = manager.register_lesson()
