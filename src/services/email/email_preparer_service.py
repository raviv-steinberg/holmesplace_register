...
class EmailPreparerService:
    ...
    def prepare_email(self, attendee_name: str, lesson: dict, seat: int) -> tuple[str, str]:
        ...
        subject = random.choice(self.subjects[lesson_type])

        # Choose a random dynamic content variation
        dynamic_content_choice = random.choice(list(self.dynamic_content_options.values()))

        # Format the body content with the appropriate variables
        dynamic_body = dynamic_content_choice["body"].format(lesson_type=lesson_type)
        ...
        <body>
            ...
            {dynamic_body}
            <p><img src="{calender_image_url}" alt="Calendar Icon" width="38" height="38" class="image-icon"/><strong class="lesson-header">Lesson Details</strong></p>
